...
class MenuBarController: NSObject {
    ...
    private var updateInterval: Double = {
        ...
        return savedInterval > 0 ? savedInterval : 3600 // Default 1 hour if not set
    }()
    ...
    private func setupMenu() {
        ...
        // Apply darker text color to the menu
    ...
    private func updateMenu(with weatherData: WeatherData) {
        ...
        // Apply dark text for better contrast
        ...
        // Current weather details - with attributed text for darker appearance
        ...
        // Make sure settings menu items also have better contrast
        if let settingsItem = menu.items.first(where: { $0.title == "Settings" }) {
            settingsItem.attributedTitle = NSAttributedString(string: "Settings", attributes: textAttributes)
        }
        
        if let quitItem = menu.items.last {
            quitItem.attributedTitle = NSAttributedString(string: "Quit", attributes: textAttributes)
        }
    }
